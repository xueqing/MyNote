...
- [ISO 基本媒体文件格式](#iso-基本媒体文件格式)
  ...
  - [附录 H (提供信息) RTP 流和接收 hint 轨道的处理](#附录-h-提供信息-rtp-流和接收-hint-轨道的处理)
    ...
    - [H.4 播放录制的 RTP 流](#h4-播放录制的-rtp-流)
      ...
      - [H.4.4 口型同步](#h44-口型同步)
      - [H.4.5 随机访问](#h45-随机访问)
    - [H.5 重新发送录制的 RTP 流](#h5-重新发送录制的-rtp-流)
      - [H.5.1 简介](#h51-简介)
...
此 box 包含采样计数和一个表格，该表给出每个采样的字节数。这允许媒体数据本身未分帧。媒体中的采样总数始终显示在采样计数中。
...
此 box 包含元数据 item 的数据，这些 item 使用构造方法指示 item 的数据区间存储在此 box 中。
...
1)采样条目的四字符代码，例如 “avc1” 被新的采样条目代码 “icpv” 代替，表示轨道不完整
...
RTP 时间戳通常不从零开始，特别是如果 RTP 接收者“调谐”到流中时。因此，应在 RTP 接收 hint 轨道中显示 Time Offset Box，并且应设置 Time Offset Box 中的 offset 值等于RTP 流接收顺序中第一个 RTP 时间戳。
...
注 2：在所有 RTP 流中，RTP 时间戳不一定随 RTP 序列号增加，即，包的传输顺序和播放顺序可能不相同。例如，许多视频编码方案都允许根据回放顺序中上一张和下一张图片进行双向预测。由于采样按照其解码顺序出现在轨道中，即 RTP 接收 hint 轨道的接收顺序，因此 rtpoffsetTLV box 中的 offset 可用于使 RTP 时间戳偏离采样时间 DT(i)。
...
可通过多种结构理解文件中的时序。影片和每个轨道都有一个**时间刻度(timescale)**。它定义了一个时间线，每秒具有多个滴答声(tick)。通过合适选择该数量，可以时间精确定时。通常，对于音频轨道这是音频采样率。对于食品，应选择合适的比例。比如，媒体时间刻度为 30000，且媒体采样时长为 1001，它准确定义了 NTSC 视频(通常，但不正确，称为 29.97)，并以 32 位提供 19.9 小时时间。(译者注：30000/1001=29.97)
...
RTP 时间戳在 RTP 接收 hint 轨道中表示为三个值的总和，其中一个值是轨道媒体时间轴中的解码时间 DT。如果采样驻留在影片片段中，则会按照组长度将解码时间编码到 Decoding Time to Sample Box 以及一个或多个 Track Fragment Run Box 中。Decoding Time to Sample Box 包含许多 sample_count 和 sample_delta 对，其中 sample_delta 是一组连续采样中每个采样的解码时间增量(即，以解码时间为单位的采样时长)，其数量等于 sample_count。Track Fragment Run Box 指示一对 sample_count 和 sample_duration，其中 sample_duration 是一组连续采样中每个采样的解码时间增量(即采样时长)，其数量等于 sample_count。每个Track Fragment Box 可以包含许多 Track Fragment Run Box。采样编号 i 的解码时间 DT(i) 是通过对 Decoding Time to Sample Box 中采样 i 之前所有采样的采样时长求和得出的，并在需要时包含 Track Fragment Run Box 中引用采样 i 之前任何采样的采样。

采样 i 的 RTP 时间戳 RTPTS(i) 由以下三个值的总和表示：(H.1)

$$RTPTS(i) = (DT(i) + tsro.offset + offset) mod 2^{32}$$

其中 tsro.offset 是引用的接收 hint 采样条目的 “tsro” box 中的 offset 值，而 offset 是 RTPpacket 结构中 rtpoffsetTLV box 中包含的值，而 mod 是取模运算。

RTP 接收 hint 采样条目中应出现 “tsro” box。轨道的任何 “tsro” box 中 offset 值应等于以 RTP 序列号顺序排列的相应流的第一个数据包的 RTP 时间戳。

假设采样 i-1 和 i 之间在最大 32 位无符号整数上未发生 RTP 时间戳值的折回，以 RTP 序列号顺序排列，连续不相等 RTP 时间戳之间的差为：(H.2)

$$对于任何 i>1, RTPTS\_DIFF(i) = RTPTS(i)-RTPTS(i-1)$$

RTPTS_DIFF(i) 保持不变，当帧速率恒定时，任何数据包中的帧数恒定，且传输顺序与显示顺序相同。音频流和时间不可缩放视频流通常会满足这些约束。如果 RTPTS_DIFF(i) 表示为 RTPTS_DIFF 的常数，则建议以下内容。Decoding Time to Sample Box 中的 sample_delta 和 Track Fragment Run Box (如果使用了影片片段) 中的 sample_duration 值设置为 RTPTS_DIFF，这将导致紧凑的 Decoding Time to Sample 和 Track Fragment Run Box。如果使用了 RTCP 接收 hint 轨道(请参阅 H.3.6)，则不应在 RTP 接收 hint 采样中使用 rtpoffsetTLV box。否则(如果不使用 RTCP 接收 hint 轨道)，则应将 rtpoffsetTLV box 中的 offset 设置为 0。
...
(RTP 序列号(SN)归一化为从 0 开始，并且假设每帧一个数据包。
RTP 时间戳(TS)归一化为从 0 开始，并表示为持续一帧间隔的时钟滴答。)

当采样 i 和 i+GOP_size (包括两个端点)之间在最大 32 位无符号整数上未发生 RTP 时间戳值的折回时，由一个 GOP 引起的 RTP 时间戳增量计算如下：(H.3)

$$RTPTS\_GOP\_DIFF(i) = RTPTS(i + GOP\_size) - RTPTS(i)$$

如果 RTPTS_GOP_DIFF(i) 是等于 RTPTS_GOP_DIFF 的常数，则当没有采样 i，i + 1，…，i + GOP_size 是从所谓的闭合图片组开始的图片时，例如 H.264/AVC 流的 IDR 图片，建议以下。Decoding Time to Sample Box 中的 sample_delta 值和 Track Fragment Run Box (如果使用了影片片段) 中的 sample_duration 值设置为 RTPTS_GOP_DIFF/GOP_size。如果使用了 RTCP 接收 hint 轨道(请参阅 H.3.6)，则 rtpoffsetTLV box 不应用于最低时间级别的图片。否则(如果不使用 RTCP 接收 hint 轨道)，则应将 rtpoffsetTLV box 中的 offset 设置为 0。应设置其他时间级别图片的 rtpoffsetTLV box 中的 offset 值，以实现公式 H.1。图 H.5 指示如何为图 H.4 中显示的分层时间可伸缩视频比特流设置解码时间和 offset。

图 H.5-在 GOP_size 等于 4 的分层时间可伸缩比特流设置解码时间(DT)和 rtpoffsetTLV box 中offset 值的示例。

(在此示例中，采样之间的解码时间增量设置为等于 RTPTS_GOP_DIFF/GOP_size，以实现紧凑的编码解码时间。为每个采样调整 rtpoffsetTLV box 中的 offset 值，以存储 RTP 时间戳的表示形式。对于这个示例，RTP 时间戳和解码时间归一化为从 0 开始，并表示为持续一帧间隔的时钟滴答。)

如果从接收到的数据包中未检测到 RTP 时间戳的线性和周期性行为，并且没有两个不同采样接收到的数据包具有相同的接收时间，建议设置 Decoding Time to Sample Box 中的 sample_delta 值和 Track Fragment Run Box (如果使用了影片片段) 中的 sample_duration 值，表示采样第一个包的接收时间。即，得到的解码时间 DT(i) 应等于采样第一包的接收时间减去流的第一接收采样的第一包的接收时间。

注意，在文件中没有为任何 hint 轨道中的采样明确指出合成时间戳。因此，对于 RTP 接收 hint 轨道，从存储的包流中指示的与 RTP 时间戳有关的信息中推断出合成时间戳。对于未关联 RTCP 接收 hint 轨道的 RTP 接收 hint 轨道，已接收 RTP 包的合成时间由采样时间 DT(i) 与包含该采样的 rtpoffsetTLV box 中的 offset 字段值之和得到。对于关联 RTCP 接收 hint 轨道的 RTP 接收 hint 轨道，合成时间推断如下。让相同轨道内具有最早 RTP 时间戳的已接收 RTP 包合成时间等于0。任何剩余 RTP 包的合成时间等于当前 RTP 包和最早(按显示顺序，其时钟漂移校正类似 H.3.6.3) RTP 包的 RTP 时间戳之差。合成时间是指轨道的媒体时间线。

#### H.3.6 为了促进回放中流间同步的记录操作

##### H.3.6.1 概述

至少可以通过以下两种方式促进口型同步，即回放过程中录制的 RTP 流之间的正确同步：

1. 为每个 RTP 接收 hint 轨道生成 RTCP 接收 hint 轨道。在解析文件并对文件中包含的媒体流进行解码和播放时，将校正不同流的 RTP 时间戳时钟之间的潜在时钟漂移。时钟漂移校正的完成方式与同时接收和播放的 RTP 流的处理方式类似。这种操作模式对于录制单元是简单直接的。但是，从精确的回放位置访问文件可能会比较麻烦，因为它需要在访问时补偿所有录制流的时钟漂移。
2. 通过修改一个或多个录制流的 RTP 时间戳，可以纠正录制的 RTP 流之间的潜在时钟漂移。这种操作模式在录制时需要处理 RTCP 发送者报告，因此，对于录制单元而言，比创建 RTCP 接收 hint 轨道更加繁琐。但是，播放器的操作很简单。

录制单元应使用 Timestamp Synchrony Box \[9.4.1.2\]指示已使用哪种口型同步方法。Timestamp Synchrony Box 包含 timestamp_sync 字段。timestamp_sync 等于 1 表示播放器应使用 RTCP 接收 hint 轨道进行口型同步。timestamp_sync 等于 2 表示播放器应使用合成时间戳记进行口型同步。

某些实现可能会在实时录制操作期间先创建 RTCP 接收 hint 轨道，然后通过修改 RTP 时间戳作为离线后处理步骤来补偿时钟漂移。

以下小节提供有关这两种方法的更多详细信息。

##### H.3.6.2 基于 RTCP 发送者报告促进口型同步

录制单元将特定 RTP 流的所有 RTCP 发送者报告存储为相应 RTCP 接收 hint 轨道中的采样。

##### H.3.6.3 补偿时间戳中的时钟漂移

不建议修改录制音频流的 RTP 时间戳。这样的修改将导致播放器中的音频时标修改，这是不平凡的操作。

视频和其他非音频流的 RTP 时间戳的记录表示形式应使用以下过程进行修改。

1. 首先，从与视频帧相对应的 RTP 时间戳导出视频帧的挂钟时间戳 a，作为前一视频帧的挂钟时间戳与 RTP 时间戳差值(当前和前一视频帧)的和，以挂钟时间线为单位。
2. 其次，基于 RTCP 发送者报告得出视频帧在挂钟时间的回放时间 b。如果没有 RTCP 发送者报告用于准确指示视频帧的挂钟时间，则可假定 RTCP 发送者报告中 RTP 时间戳时钟和发送者挂钟的偏离速率保持不变，从而推断挂钟时间。
3. 第三，基于音频的 RTCP 发送者报告，得出在挂钟时间线的时间 b 上与视频帧同时播放的音频 RTP 时间戳。不需要音频帧具有确切得到的音频 RTP 时间戳。从派生的音频 RTP 时间戳计算音频采样的挂钟时间戳 c，作为前一音频帧的挂钟时间戳与派 RTP 时间戳差值(当前和前一音频帧)的和。
...
如 9.4.1.4 规定，包的接收时间表示为包含该包采样的解码时间与包的 RTPpacket 结构的 relative_time 值之和。

最早已接收 RTP 包的接收时间应为零，所有后续包的接收时间应相对于最早已接收 RTP 包的接收时间。

接收时间的时钟源是不确定的，例如可以是接收者的挂钟。如果一个接收 hint 轨道的接收时间范围与另一个接收 hint 轨道的接收时间范围全部或部分重叠，则这些 hint 轨道的时钟源应相同。

包的接收时间对应的时刻，是 RTP 下的协议堆栈层(通常为 UDP)输出数据包的时间。

#### H.3.8 媒体采样的创建

根据相关的 RTP 有效负载规范和 RTP 本身的指示，从接收到的 RTP 包中创建媒体采样。但是，大多数媒体编码标准仅指定无错误流的解码，因此应确保任何符合标准的媒体解码器都能正确解码媒体轨道中的内容。因此，处理传输错误需要两个步骤：检测传输错误和推断可以正确解码的采样。这些步骤在后续段落进行描述。

可以从 RTP 序列号值的间隔检测到丢失的 RTP 包。包含位错误的 RTP 包通常不转发到应用程序，因为它们的 UDP 校验和失败，并且包在接收者的协议栈中被丢弃。因此，位比特的包通常在接收者被视为包丢失。

推断可以正确解码的媒体采样取决于媒体编码格式，因此在此不再详细描述。通常，采样间预测在音频编码格式中比较弱或不存在，而大多数视频编码格式大量利用帧间预测。因此，许多音频格式中丢失的采样通常可以用无声或隐藏错误的音频采样代替。应该分析丢失的视频包是否关联非参考图片或参考图片，或更一般地说，丢失发生在时间可伸缩层次的哪个级别上。然后应该得出结论，哪些图片可能无法正确解码。例如，非参考图片的丢失不会影响任何其他图片的解码，而丢失基本时间级别的参考图片通常会影响所有图片，直到下一个用于随机访问的图片，例如 H.264/AVC 中的 IDR 图片。视频轨道不得包含任何依赖丢失的视频采样的采样。

#### H.3.9 创建引用媒体采样的 hint 采样

如 H.3.8 所述，根据接收到的 RTP 包创建媒体采样。如 H.3.4 所述创建了 RTP 接收 hint 轨道，但是 RTPpacket 结构的内容取决于对应媒体采样的存在，如下所示。

如果在媒体轨道中表示已接收 RTP 包的包有效载荷，则将相关包构造器的轨道引用设置为指向媒体轨道，并通过引用包含包有效载荷。不建议在已接收 RTP 采样的 extradata 部分中复制包有效负载，以节省存储空间并使文件编辑操作更易于实现。

如果未在媒体轨道中表示已接收 RTP 包的包有效载荷，则如 H.3.4 所述创建 RTPpacket 结构的实例。

### H.4 播放录制的 RTP 流

#### H.4.1 简介

本小节描述了回放包含录制 RTP 流的文件所需的操作。它的组织如下：

- 在播放 RTP 流之前，应先分析文件的内容。特别是，表示应该识别相同媒体流的可选轨道，并且应选择这些轨道中的一个用于解码和回放。为了预先推断出可以被播放器解码，应该检测编码格式。这些准备操作在 H.4.2 中有更详细的描述。
- 如果正在处理 RTP 接收 hint 轨道，则应考虑 0 所述的一些事项。例如，应检测并适当处理包丢失。
- 应正确处理解码的媒体采样的同步，如 0 所述。
- 如果从流的开头以外的位置访问文件中存储的 RTP 流，则需要进行适当的流间同步和解码器初始化，如 H.4.5 所述。

#### H.4.2 回放准备

在回放准备阶段，播放器选择要播放的轨道。首先解析文件的基本轨道结构。根据轨道所属的备用组对其进行分组。属于相同备用组的轨道通过 Track Header Box 中 alternate_group 的相同值指示。从每个备用组中选择一个轨道进行回放，如下所示。

如果备用组中有 RTP 接收 hint 轨道，则优先用于回放，因为它包含已接收 RTP 流的完整表示，不同于从已接收 RTP 流派生的媒体轨道，媒体轨道可能会使用已接收 RTP 包的子集，这些包可由任何符合标准但不能处理数据包丢失的解码器解码。

应确保播放器与所选轨道的兼容性。例如，应该检查编解码器、配置和轨道中使用的级别是否被播放器支持。

可以从 RTP 流的 SDP 描述中得出 RTP 接收 hint 轨道中已编码比特流所用的编解码器、配置和级别。SDP 描述存储在影片级索引轨道中。如果整个文件中的 SDP 不变，则可以在 User Data box 中将其另存为影片 SDP 信息和轨道 SDP 信息。如果存在轨道 SDP 信息，则可以对其进行解析找出 RTP 接收 hint 轨道中包含的比特流所用的编解码器、配置和级别。如果不存在影片 SDP 信息或轨道 SDP 信息，则遍历影片级索引轨道以查找和解析每个 SDP 索引，并因此查找和解析 RTP 接收 hint 轨道中包含的比特流所用的编解码器、配置和级别。

如果备用组中不存在 RTP 接收 hint 轨道，则应检查备用组中采样条目或媒体轨道的采样条目，以找出播放器能够支持的采样。

#### H.4.3 对 RTP 接收 hint 轨道中的采样进行解码

可以通过从 RTPpacket 结构创建 RTP 包头以及解析 RTPpacket 结构的构造器，从 RTP 接收 hint 采样中重建原始 RTP 包。因此，文件播放器处理 RTP 接收 hint 轨道的一种方法是重新创建接收到的包流，并像新接收到的那样处理重新创建的包流。

RTPpacket 结构中包含的 relative_time 字段可用于安排将包插入 RTP 接收者的缓冲区。但是，更可取的做法是，修改录制 RTP 流的解码过程，以使解码器输出缓冲区保持尽可能满，以避免在运行除播放器之外的其他进程的系统中，由于后期包或实时解码中偶尔出现的问题造成的中断或跳动回放。

应当从 RTP 序列号间隔中检测包丢失。对包丢失的反应取决于特定媒体解码器的实现，也可能取决于用户的偏好。
...
1. 播放开始时轨道间的同步。
轨道媒体时间线的起始位置可以在文件的影片时间线中移动，如以下两段所述。
对于媒体轨道和未关联 RTCP 接收 hint 轨道的 RTP 接收 hint 轨道，应使用 Edit List Box 移动影片时间线内媒体时间线的起始位置，如 H.3.2 所述。通过分析轨道的 Edit List Box(如果存在)，将选定回放的轨道的媒体时间线映射到影片时间线。每个媒体轨道和与 RTCP 接收 hint 轨道不相关的每个 RTP 接收 hint 轨道的回放，均从该轨道 Edit List Box 中指示的影片时间线位置开始，或者从影片时间线的开头开始(如果该轨道没有 Edit List Box)。
对于关联到对应 RTCP 接收 hint 轨道的 RTP 接收 hint 轨道，影片时间线内媒体时间线的起始位置的偏移如下推断。在所有 RTP 接收 hint 轨道中，包含最早 RTP 包(在发送方挂钟时间线上的显示时间)的 RTP 接收 hint 轨道的媒体时间线不会在影片时间线内偏移(即，在影片时间线上的时间 0 开始)。任何其他 RTP 接收 hint 轨道的媒体时间线的开始时间等于当前轨道与所有 RTP 接收 hint 轨道中包含最早 RTP 包的轨道的最早 RTP 包的时间戳差值。
2. 在媒体时间线上重构 RTP 时间戳和合成时间(H.3.5)。
3. 如果使用 RTCP 接收 hint 轨道，则基于 RTCP 发送者报告校正 RTP 时间戳和合成时间。
校正与 H.3.6.3 中描述的类似。但是，不是将时间 a 和 c 之间的差添加到文件中 RTP 时间戳的表示中，而是在回放过程中将时间差添加到影片时间线上视频帧的显示时间。
4. 调整解码的媒体采样的输出速度。
建议以播放器的挂钟速度播放录制的节目，并使用音频播放时钟作为播放器的挂钟。安排音频回放在音频信号的原始采样频率下是连续的。播放器的显示时钟以音频播放的速度运行，即其值始终等于(播放的最频繁的未压缩音频采样的数量)×(音频信号的采样频率)。视频轨道(和其他潜在的连续媒体轨道)的回放与播放器的显示时钟同步。换句话说，当播放器的显示时钟在影片时间线上满足视频采样的合成时间时，将播放视频采样。
仅当同时录制和播放文件，并且接收者挂钟的运行速度比发送器挂钟的运行速度快时，才可能不建议根据接收者挂钟的速度调整播放速度，并可以按以下步骤将接收者挂钟的速度同步到接收者挂钟的速度。
通过建立接收时间(根据接收者时钟)和对应发送者挂钟时间戳的关系，来恢复发送者时钟的速度，发送者挂钟时间戳是从 RTCP 发送者报告中重建的。建议使用音频播放时钟作为接收者时钟。由于网络和接收者中的延迟可能会发生变化，因此应在大量已接收包上平均接收时间与对应发送者时间戳之间的关系。通过平均接收时间与对应发送者时间戳之间的关系，可以得出时间刻度乘数因子。
对于每个采样，得出接收者时钟时间线上的显示时间。如果正在使用 RTCP 接收 hint 轨道，则显示时间是影片时间线上的采样合成时间，还包括上面步骤 3 所述的时钟漂移校正。如果未使用 RTCP 接收 hint 轨道，则显示时间直接是影片时间线上采样的合成时间。然后，仅出于回放目的，应将所有正在回放轨道的采样的显示时间乘以时间刻度乘数因子。
应相应地完成信号的时间延长。在采样的显示时间播放采样。
实际上，时间刻度乘数因子和从 RTP 时间线到发送者挂钟的映射(上述步骤 3)可以实现为单个操作。

#### H.4.5 随机访问

随机访问是指非线性访问文件中表示的媒体流。换句话说，在随机访问操作中，从不是先前回放的另一个采样访问文件，或者从不是影片时间线开始的位置初始访问文件。

建议为用户提供相对于文件的影片时间线的随机访问功能，而不是相对其他时间线(例如发送者挂钟时间线)提供随机访问功能。通过以影片时间线为基础，将随机访问操作的步骤数保持为低水平。

首先，得到哪些媒体帧在期望的随机访问位置(或者，如果没有媒体帧恰好在期望的随机访问位置，则最接近的)。对于媒体轨道，音频的 RTP 接收 hint 轨道以及 timestamp_sync 字段等于2(指示预补偿口型同步)的任何 RTP 接收 hint 轨道，可以基于合成时间戳(在媒体时间线上)直接得到最接近期望的随机访问位置的媒体帧，该合成时间戳通过 Edit List Box (如果有)中指示的初始开始位置偏移。对于 timestamp_sync 字段等于 1 (指示使用 RTCP 接收 hint 轨道)的非音频 RTP 接收 hint 轨道，应按照 0 的描述得到采样的显示时间，直到找到与期望的随机访问点最近的显示时间。

其次，许多媒体比特流的解码只能从特定类型的帧开始，例如 H.264/AVC 的 IDR 图片。因此，播放器实现可能具有不同的方法，包括以下方法：

1. 发现期望的随机访问位置(可以从其开始解码)处或其之前的最近帧，从该帧开始解码，然后仅从期望的随机访问点开始渲染。此方法可能暗示开始渲染之前有一些处理延迟。
2. 使用可从其开始解码的最早帧，在期望的随机访问点处或之后开始解码和渲染。通常，音频回放将比视频回放更早开始，但是开始渲染之前的处理延迟小于前一个选项中的延迟。

### H.5 重新发送录制的 RTP 流

#### H.5.1 简介

将先前录制的 RTP 流重新发送到文件可能是期望的操作。例如，如果通过广播或流服务接收 RTP 流并将其录制到文件中，则可能期望使用 WLAN 连接将它们从家庭环境中的一个设备重新发送到另一设备。本小节提供建议用于重新发送录制的 RTP 流。

基于 RTP 的通信系统包括源端点(也称为发送者)和目的端点(也称为接收者)，并且可以包含一个或多个混合器和转换器。发送者和接收者是 RTP 和 RTCP 会话的端点。 RTP 转换器和混合器的行为在 RFC 3550 中指定，并在 RFC 5117 澄清。通常，接收 RTP 流并将其存储到文件中的录制单元充当目标端点，而重发单元从文件中读取存储的 RTP 流，并作为源发送它们。通常，重新发送 RTP 流的有效载荷不会被修改，这使得录制单元和重发单元的组合类似于 RFC 5117 所述的传输转换器。但是，转换器的本质特征是：接收者无法检测到它的存在。因此，除非重新发送与原始流的录制同时发生，否则录制单元和重发单元的组合不能充当传输转换器。由于这种情况很少见，因此本小节中的讨论将录制单元视为终止原始 RTP 和 RTCP 会话的目的地，并将重发单元视为新的 RTP 和 RTCP 会话的源。

本小节的组织如下：

- H.5.2 包括如何从 RTP 接收 hint 轨道组成 RTP 包以及如何安排 RTP 包传输的建议。
- H.5.3 讨论如何生成 RTCP 包以及如何处理接收到的 RTCP 包。

#### H.5.2 重新发送 RTP 包

建议按照以下方式构造和传输包。

建议根据存储在接收 hint 轨道中的构造器构造包的有效载荷，即，建议数据包有效载荷与接收到的有效载荷相同，除非不同的数据包大小对于重新发送数据包的网络至关重要。

- 除以下情况外，由 RTP 接收 hint 轨道建议的 RTP 包头字段的值，应与对应的 RTPpacket 结构中的值保持相同：
- 应该随机选择初始 RTP 时间戳偏移和 RTP 序列号偏移，而无关存储在引用接收 hint 采样条目的 “tsro” box 的 offset 字段中的值，或相应 RTP 接收 hint 轨道任何包的 RTPpacket 结构的 RTPsequenceseed 字段的值如何。
- RTP 时间戳字段的值应为随机初始偏移量、RTPpacket 结构中的 offset 值，以及对应 RTP 采样的解码时间之和。如果总和超过最大的 32 位无符号整数，则应将其包装。
- RTP 序列号的相对增量应与 RTPsequenceseed 字段值中记录的相对增量值相同。因此，如果录制的流中有包丢失，则重新发送的流在 RTP 序列号中也有间隔，并且接收者能够推断出数据包丢失。
- CSRC 计数字段的值应始终为零，因为录制的前一个 RTP 会话的贡献源都没有为正在重发的流主动修改 RTP 会话的流。源标识符空间(用于 SSRC 和 CSRC)是特定于会话的。因此，RTP 头的 CSRC 列表应该为空，而不考虑接收流的潜在存储的 CSRC 值，这些值包括在 RTPpacket 结构的 receiveCSRCTLV box 中。
- 可以根据使用中的信令方案动态选择有效载荷类型字段的值。
- 应该随机选择 SSRC 字段的值，并应按照 RFC 3550 中的规定处理潜在的冲突。已接收流的 SSRC 值可以存储在引用的接收 hint 采样条目的 ReceivedSsrcBox 中，但是当重新发送流时应忽略。
- 仅在重发单元可以验证它们对重新发送的流有效时，才应重新发送存储在 RTPpacket 结构(如果有)中 rtphdrextTLV 中录制的 RTP 头部扩展。如果重发单元无法解析录制的 RTP 头部扩展的语义，则不应重新发送它们。

包的接收时间表示为包含该包的 RTP 接收 hint 采样的解码时间与 RTPpacket 结构的 relative_time 值的总和，等于该包的传输时间，具有因传输延迟和接收者协议栈中的处理延迟导致的偏移。由于传输延迟抖动和处理延迟的变化，相邻数据包的偏移可能不相等。此外，接收流时使用的协议栈可能不同于重新发送流使用的协议栈。由于这些原因，接收时间通常不适用于调整重新发送包的传输速度。在所有情况下，重发单元都应验证重新发送的包流符合使用的缓冲模型(如果有)。如果重发单元可以断定接收流和重新发送录制流时使用的网络环境和协议栈是相似的，则接收时间可以用作调度包传输的基础。重发单元应努力消除或隐藏录制流中的传输延迟抖动。如果重发单元无法断定接收流和重新发送录制流时使用的网络环境和协议栈相似或不确定合适哪种包调度，则可以将解码时间用作调度的基础。

#### H.5.3 RTCP 处理

RTCP 发送者报告和其他 RTCP 消息按照 RFC 3550 中指定的约束重新生成，而不是直接使用记录在 RTCP 接收 hint 轨道(如果有)中的 RTCP 消息。

RTCP 发送者报告包含发送报告的挂钟时间，以及与指示的挂钟时间相同的时间对应的 RTP 时间戳。RTCP 发送者报告的 RTP 时间戳生成如下。对于 RTCP 发送者报告中指示的挂钟时间相对应的采样，得出参考时钟时间线上的显示时间。参考时钟可能是重发单元的挂钟，其会话开始时初始化为 0。由于 RTP 接收 hint 轨道中采样的采样时刻可能不匹配 RTCP 发送者报告的传输时刻，因此与指示的挂钟时间对应的采样可能不存在于相应的 RTP 接收 hint 轨道中。但是，按照 RFC 3550 的指示，RTP 时间戳的推导就像在 RTP 流中存在一个采样与指示的挂钟时间相对应。RTCP 发送者报告的 RTP 时间戳应从 RTCP 发送者报告中指示的挂钟时间之前和之后采样的 RTP 时间戳进行线性插值。为了得出 RTCP 发送者报告中指示的挂钟时间之前和之后的采样，应该得出参考时钟时间线上的显示时间，直到发现最近的采样。如果重发的 RTP 接收 hint 轨道存在 RTCP 接收 hint 轨道，则显示时间是影片时间线上采样的合成时间，还包括 0 中步骤 3 所述的时钟漂移校正。如果不存在 RTCP 接收 hint 轨道，显示时间直接是影片时间线上采样的合成时间。

在处理接收到的 RTCP 接收者报告时，应注意，报告的丢失包累积数量还包括未发送的包，这些包最初从未接收过，并且对应于 RTP 接收 hint 轨道中 RTP 序列号的间隔。任何拥塞管理、重传或其他丢包恢复方法都应考虑到这一点。

## 附录 I (提供信息) 流访问点

### I.1 简介

此附录定义了流访问点(SAP)并指定了六种 SAP 的类型。

流访问点(SAP)允许随机访问媒体流的容器。一个容器可以包含多个媒体流，每个媒体流都是某些媒体类型的连续媒体的编码版本。SAP 是容器中的一个位置，允许仅使用以下内容开始回放标识的媒体流：(a)从该位置开始包含在容器中的信息，以及(b)来自容器其他部分或外部可用的可能的初始化数据。派生的规范应指定是否需要初始化数据访问 SAP 处的容器，以及如何访问初始化数据。

分层媒体的 SAP 可以应用于媒体流中的所有层、特定的一组层或仅单个层。当 SAP 应用于一组层，这组层使用来自非集合成员的层的帧间预测，可能会指示 SAP 是否需要对引用层进行正确解码。

当 SAP 与分层媒体一起使用时，派生的规范应指定或提供方法指示 SAP 适用的层，以及 SAP 是否需要对引用层进行正确解码。

### I.2 SAP 属性

#### I.2.1 概述

对于每个 SAP，标识和定义 Isap、Tsap、Isau、Tdec、Tept 和 Tptf 属性为：

- Tsap 是媒体流中任何访问单元的最早显示时间，因此，可以正确解码媒体流所有显示时间大于或等于 Tsap 的访问单元，使用比特流从 Isap 开始的数据且不使用 Isap 之前的数据。
- Isap 是比特流中的最高位置，因此，可以正确解码媒体流所有显示时间大于或等于 Tsap 的访问单元，使用比特流从 Isap 开始的数据且不使用 Isap 之前的数据。
- Isau 是比特流中，按解码顺序，媒体流中最新访问单元的起始位置，因此，可以正确解码媒体流所有显示时间大于或等于 Tsap 的访问单元，使用此最新访问单元和其后按解码顺序排列的访问单元，不使用按解码顺序早于该最新访问单元的访问单元。
注意 Isau 始终大于或等于 Isap 。
- Tdec 是媒体流任何访问单元的最早显示时间，可以正确解码媒体流，使用比特流中从 Isau 开始的数据且不使用 Isau 之前的数据。
- Tept 是媒体流任何访问单元的最早显示时间，媒体流在比特流中从 Isau 开始。
- Tptf 是媒体流第一个访问单元的显示时间，媒体流是从 Isau 开始的比特流中按解码顺序排列的。

为了这些定义的目的，SAP 被描述为位于 Isau 和/或 Isap 的访问单元。

注意：仅需要在 Isau 和 Isap 之间进行区分才能区分直接引用访问单元和引用其包含结构。

#### I.2.2 图层的 SAP 属性

以下属性适用于分层媒体流，针对该分层媒体流指示了一个或多个层(称为目标层)的 SAP。在以下属性中，访问单元分区指的是一个单元，该单元包含目标层单个时间实例的编码的数据，媒体流分区是指按解码顺序，目标层的访问单元分区序列。

当目标层覆盖媒体流的所有层时，以下属性与 I.2.1 中的属性相同。

对于每个 SAP，标识和定义 Isap、Tsap、Isau、Tdec、Tept 和 Tptf 属性为：

- Tsap 是目标层任何访问单元分区的最早显示时间，因此可以正确解码目标层所有显示时间大于或等于 Tsap 的访问单元分区，使用从 Isap 开始的媒体流分区中的数据且不使用 Isap 之前的数据。
- Isap 是媒体流分区容器中的最高位置，因此可以正确解码目标层所有显示时间大于或等于 Tsap 的访问单元分区，使用从 Isap 开始的媒体流分区中的数据且不使用 Isap 之前的数据。
- Isau 是媒体流分区中，按解码顺序，最新访问单元分区的起始位置，因此可以正确解码目标层所有显示时间大于或等于 Tsap 的访问单元分区，使用此最新访问单元分区和其后按解码顺序排列的访问单元分区，不使用按解码顺序早于该最新访问单元分区的访问单元分区。
注意 Isau 始终大于或等于 Isap。
- Tdec 是目标层任何访问单元分区的最早显示时间，目标层可以正确解码，使用从 Isau 开始的媒体流分区中的数据且不使用 Isau 之前的数据。
- Tept 是目标层任何访问单元分区的最早显示时间，目标层在媒体流分区中从 Isau 开始。
- Tptf 是目标层第一个访问单元分区的显示时间，目标层是从 Isau 开始的媒体流分区中按解码顺序排列的。

### I.3 SAP 类型

定义了六种类型的 SAP，其属性如下：

- 类型 1： Tept = Tdec = Tsap = Tptf
- 类型 2： Tept = Tdec = Tsap < Tptf
- 类型 3： Tept < Tdec = Tsap <= Tptf
- 类型 4： Tept <= Tptf < Tdec = Tsap
- 类型 5： Tept = Tdec < Tsap
- 类型 6： Tept < Tdec < Tsap

注意 SAP 的类型仅取决于可正确解码的访问单元及其在显示顺序中的排列。这些类型非正式地与一些通用术语相对应：

- 类型 1 对应于某些编码方案中所谓的“闭合 GoP 随机访问点”(其中，从 Isap 开始，按照解码顺序，所有访问单元都可以正确解码，从而生成一个正确解码的没有间隔的连续时间序列)，此外，按解码顺序的访问单元也是显示顺序的第一个访问单元。
- 类型 2 对应于某些编码方案中所谓的“闭合 GoP 随机访问点”，对于该类型，从 Isau 开始，媒体流中按解码顺序排列的第一个访问单元不是按显示顺序排列的第一个访问单元。
- 类型 3 对应于某些编码方案中所谓的“开放 GoP 随机访问点”，其中在 Isau 之后，按解码顺序有一些访问单元无法正确解码，并且显示时间小于 Tsap 。
- 类型 4 对应于某些编码方案中所谓的“逐步解码刷新(GDR)随机访问点”，其中在 Isau 的前后，按解码顺序有一些访问单元无法正确解码，并且显示时间小于 Tsap 。
- 类型 5 对应于以下情况：从 Isap 开始，至少有一个按解码顺序的访问单元无法正确解码，并且显示时间大于 Tdec，Tdec 是从 Isau 开始的任何访问单元的最早显示时间。
- 类型 6 对应于以下情况：从 Isap 开始，至少有一个按解码顺序的访问单元无法正确解码，并且显示时间大于 Tdec，Tdec 不是从 Isau 开始的任何访问单元的最早显示时间。
