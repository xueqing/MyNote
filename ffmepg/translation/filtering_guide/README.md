...
参考 [FFmpeg 过滤器文档](http://ffmpeg.org/ffmpeg-filters.html)获取每个过滤器的更多信息和示例。此维基页面用于用户提供示例和技巧，并且鼓励所有人优化此页面。

## 过滤器语法

当文档提及`过滤器选项`，或者说`过滤器接受下面的选项`，这些选项的用法正如 “FFmpeg 过滤器: 4.1 [Filtergraph](http://ffmpeg.org/ffmpeg-filters.html#Filtergraph-syntax-1) 语法”章节中描述的。简而言之，在过滤器名称之后，添加一个 `=`，然后是第一个过滤器选项的名字，一个 `=`，接着是该选项对应的值。如果想要指定更多选项，使用 `:` 分隔符，然后追加下一个选项的名称，一个 `=`，然后是这个新选项的值。

比如，将 [loudnorm 过滤器](http://ffmpeg.org/ffmpeg-filters.html#loudnorm)应用到一个音频流，基本语法是：
...
正如 “FFmpeg 过滤器: 4.1 [Filtergraph](http://ffmpeg.org/ffmpeg-filters.html#Filtergraph-syntax-1) 语法”章节中描述的，你可以省略选项名称和 `=`，只提供选项值，国歌选项值用`:` 分隔。比如，下面的命令：

```sh
ffmpeg -i input -vf scale=iw/2:-1 output
```

FFmpeg 按照源代码中声明的选项顺序匹配选项名。比如，在上述 [scale 过滤器](http://ffmpeg.org/ffmpeg-filters.html#scale)的使用示例中，FFmpeg 认为 `width` 选项的值是 `iw/2`，`height` 选项的值是 `-1`。

### Filtergraph、Chain、Filter 的关系

ffmpeg 命令行中跟在 `-vf` 之后的是 [filtergraph](http://ffmpeg.org/ffmpeg-filters.html#Filtergraph-description) 描述。这个 filtergraph 可以包含多个 chain，每个 chain 可以包含多个 filter。
...
请注意，文档中给出的示例混合匹配使用了“全引号”和“\”转义，并且在特殊的 shell 使用转义可能比较复杂。有关更多信息，参考 [filtergraph 转移的注意事项](http://ffmpeg.org/ffmpeg-filters.html#Notes-on-filtergraph-escaping)。
...
`iw` 是输入的宽度。在这个示例中输入宽度是 640，且 640/2=320。`-1` 告诉 scale 过滤器保留输出的原长宽比，因此这个示例中 scale 过滤器选择 240.有关更多信息查看 [FFmpeg 文档](http://ffmpeg.org/ffmpeg-filters.html#scale)。
...
这里使用 `-filter_complex` 选项将 4 个输入一起过滤。在这个示例中，所有输入都是 `-f lavfi -i testsrc`(查看 [testsrc 源过滤器](https://ffmpeg.org/ffmpeg-filters.html#color_002c-haldclutsrc_002c-nullsrc_002c-rgbtestsrc_002c-smptebars_002c-smptehdbars_002c-testsrc))，但也可以是其他输入。

在 filtergraph 内部，第一个输入没有更改，其他三个输入分别使用 [hflip](https://ffmpeg.org/ffmpeg-filters.html#hflip)、[negate](https://ffmpeg.org/ffmpeg-filters.html#negate) 和 [edgedetect](https://ffmpeg.org/ffmpeg-filters.html#edgedetect) 过滤。然后使用 hstack 和 vstack 过滤器将每个视频入栈到所需的位置。
...
下一个示例等同于上述示例，但是使用了 [pad](https://ffmpeg.org/ffmpeg-filters.html#pad) 和 [overlay](https://ffmpeg.org/ffmpeg-filters.html#overlay-1) 过滤器。pad 过滤器用于生成合适大小的背景，而 overlay 过滤器用于将每个视频放在正确的位置。这个方法比上述使用 `hstack+vstack` 慢：
...
使用 [drawtext](http://ffmpeg.org/ffmpeg-filters.html#drawtext-1) 视频过滤器。
...
[testsrc 源过滤器](http://ffmpeg.org/ffmpeg-filters.html#color_002c-haldclutsrc_002c-nullsrc_002c-rgbtestsrc_002c-smptebars_002c-smptehdbars_002c-testsrc)生成一个测试视频模式，该模式显示一个颜色模式，一个滚动渐变和一个时间戳。它在测试时非常有用。
...
testsrc 的另一种类型是使用 [smptebars 源过滤器](http://ffmpeg.org/ffmpeg-filters.html#color_002c-haldclutsrc_002c-nullsrc_002c-rgbtestsrc_002c-smptebars_002c-smptehdbars_002c-testsrc)：
...
过滤器可以写入元数据条目，这些条目可用于调试过滤器的功能，或者从输入文件中提取额外的信息。通用描述可查看[元数据过滤器文档](http://ffmpeg.org/ffmpeg-filters.html#metadata_002c-ametadata)。
...
假定你想要基于一个输入文件修改一些过滤器参数。一些过滤器支持使用 [sencmd](https://ffmpeg.org/ffmpeg-filters.html#sendcmd_002c-asendcmd) 选项接受命令。运行 `ffmpeg -filters` 并检查 `C` 列——如果存在 `C`，该过滤器支持这种方式输入。
